...
function [s taskSpecific] = elirlcdsparseencode(model,algorithm_params,theta,D)
groupid = algorithm_params.group_id;
Dsqrt = real(D^.5);
suppressTaskSpecific = isinf(algorithm_params.muRatio);
target = Dsqrt * theta;
muProd = algorithm_params.mu_1 * algorithm_params.muRatio;
dictTransformed = Dsqrt * model.Psi{groupid} * model.L;
if ~suppressTaskSpecific
	designMatInv = inv(Dsqrt' * Dsqrt + muProd * diag([ones(size(model.L,1)-1,1); 0])) * Dsqrt';
end
converged = 0;
taskSpecific = zeros(model.dg(groupid),1);
...
loss = Inf;
while ~converged
	s = full(mexLasso(target - Dsqrt*taskSpecific,dictTransformed,struct('lambda',algorithm_params.mu_1/2)));
	...
	lossOld = loss;
	% evaluate loss
	loss = algorithm_params.mu_1.*norm(s,1)+sum((target - dictTransformed*s - Dsqrt*taskSpecific).^2)+muProd*sum(taskSpecific.^2);
