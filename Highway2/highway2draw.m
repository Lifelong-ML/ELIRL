% Draw single highway2 with specified reward function.
function highway2draw(r,p,g,mdp_params,mdp_data,feature_data,model)
...
% Action highway2 to gridworld conversion table.
actionmap = [ 5 3 4 2 1 ];
    ...
    for x=1:length,
        for lane=1:lanes,
            for spd=1:speeds,
                ...
                s = highway2coordtostate(x,lane,spd,mdp_params);
                a = p(s);
                % Convert a from highway2 action to gridworld action.
                a = actionmap(a);
