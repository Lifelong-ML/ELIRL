% Construct the raw features for the highway2 domain.
function [feature_data,true_feature_map,r] = highway2features(mdp_params,mdp_data)
...
mdp_params = highway2defaultparams(mdp_params);
...
tests = mdp_params.speeds+mdp_params.lanes+(mdp_params.c1+mdp_params.c2)*64;%+(mdp_params.c1+mdp_params.c2)*16*mdp_params.speeds;
...
for s=1:mdp_data.states,
    [x,lane,speed] = highway2statetocoord(s,mdp_params);
    if lane == mdp_params.fav_lane
        r(s,:) = r(s,:) + mdp_params.r_lane;
    end
    if speed == mdp_params.fav_speed
        r(s,:) = r(s,:) + mdp_params.r_speed;
    end
    ...
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,lane,1);
    strti = strti+cnt;
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,lane,-1);
    ...
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,max(1,lane-1),1);
    strti = strti+cnt;
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,max(1,lane-1),-1);
    ...
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,min(mdp_params.lanes,lane+1),1);
    strti = strti+cnt;
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,min(mdp_params.lanes,lane+1),-1);
    ...
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,-1,1);
    strti = strti+cnt;
    strtci = strtci+ccnt;
    [splittable(s,strti+1:strti+cnt),splittablecont(s,strtci+1:strtci+ccnt)] = ...
        highway2closestcar(mdp_data,mdp_params,x,-1,-1);
%     strti = strti+cnt;
%     splittable(s,strti+1:end) = reshape(splittable(s,1:mdp_params.speeds)'*splittable(s,idx:idx-1+2*cnt),...
%         1,(mdp_params.c1+mdp_params.c2)*16*mdp_params.speeds);
end;
