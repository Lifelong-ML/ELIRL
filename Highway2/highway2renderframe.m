% This is an alternative function for drawing the highway2 environment.
% Instead of rendering the entire environment, with different speeds shown
% side-by-side, this function renders a specified portion of the highway2
...
function highway2renderframe(r,mdp_params,mdp_data,hlength,orientation,...
    s1,s2,trans_frac,draw_agent,quality)
% r - the reward function of this environment.
% mdp_params - highway2 parameters.
% mdp_data - data for this MDP.
% length - how much of the highway2 to draw.
...
% Get state positions.
[x1,lane1,speed1] = highway2statetocoord(s1,mdp_params);
[x2,lane2,speed2] = highway2statetocoord(s2,mdp_params);
...
if ~isempty(r),
    ...
    for y=floor(startx-0.01):ceil(startx+hlength+0.01),
        for l=1:lanes,
            ...
            s = highway2coordtostate(yy,l,speed1,mdp_params);
            % Draw reward.
            if rngr == 0,
                v = 0.0;
            else
...
for i=1:size(mdp_data.c1array,1),
    for j=1:size(mdp_data.c1array{i},1),
        ...
        if y > startx && y-1 < startx+hlength,
            % Draw the car.
            highway2renderobject(lane,y,c1,c2,quality);
        end;
        if y2 > startx && y2-1 < startx+hlength,
            % Draw the car.
            highway2renderobject(lane,y2,c1,c2,quality);
        end;
        if y3 > startx && y3-1 < startx+hlength,
            % Draw the car.
            highway2renderobject(lane,y3,c1,c2,quality);
        end;
    end;
end;
...
% Draw the agent.
if draw_agent,
    highway2renderobject(al,ax,3,1,quality);
end;
