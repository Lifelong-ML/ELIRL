...
function [irl_result, st_model] = maxentrun(algorithm_params,mdp_data,mdp_model,...
...
st_model.time = time;
st_model.theta = wts;
st_model.hesseye = elirlmaxenthessian(struct('hess_approx','eye'),F,soln,mdp_data,initD,example_samples);
tic;
st_model.hesscov = elirlmaxenthessian(struct('hess_approx','use_cov'),F,soln,mdp_data,initD,example_samples);
time = toc;
st_model.hess_time = time;
