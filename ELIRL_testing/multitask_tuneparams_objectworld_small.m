...
for i = 1:num_tasks
    if i == 1
        ...
        [mdp_data{i}, r{i}, feature_data{i}, true_feature_map{i},mdp_params_all{i}] = feval(strcat(mdp,'build'),mdp_params, color_placement);
    end
...
% tmp = load('./dumps/maxent_objectworld2_22-Jan-2018_081257.mat');
...
avg_val_elirl = zeros(length(mu),length(lambda),length(ks),num_perms);
avg_val_diff_elirl = zeros(length(mu),length(lambda),length(ks),num_perms);
avg_r_diff_elirl = zeros(length(mu),length(lambda),length(ks),num_perms);
err_val_elirl = zeros(length(mu),length(lambda),length(ks),num_perms);
err_val_diff_elirl = zeros(length(mu),length(lambda),length(ks),num_perms);
err_r_diff_elirl = zeros(length(mu),length(lambda),length(ks),num_perms);
test_result_elirl = cell(length(mu),length(lambda),length(ks),num_perms);
...
for j = 1:num_perms
    for l = 1:length(lambda)
        for m = 1:length(mu)
            for k = 1:length(ks)
                algorithm_params = struct('k',ks(k),'mu',mu(m),'lambda',lambda(l),'hess_approx','use_cov','initializeWithFirstKTasks',true);
                % Init Model
                test_params.training_samples = training_samples;
                tasks.train_percent = percentages_trained;
                [test_result{m,l,k,j}] = runmultitaskxval('elirl',algorithm_params,mdp_model,...
                    mdp,mdp_params_all,test_params, tasks,st_model(permutations(j,:)),permutations(j,:), n_folds);
            end
        end
    end
end
...
val_elirl = zeros(length(mu),length(lambda), length(ks),num_tasks,num_perms);
val_diff_elirl = zeros(length(mu),length(lambda),length(ks),num_tasks,num_perms);
r_diff_elirl = zeros(length(mu),length(lambda),length(ks),num_tasks,num_perms);
...
for j = 1:num_perms
    for l = 1:length(lambda)
        for m = 1:length(mu)
            for k = 1:length(ks)
                for i = 1:num_tasks
                    r_diff = test_result{m,l,k,j}{i}.metric_scores{2,8};
                    val = test_result{m,l,k,j}{i}.metric_scores(2,4);
                    value = val{1}(2);
                    diff_val = val{1}(1);
                    val_elirl(m,l,k,i,j) = value;
                    val_diff_elirl(m,l,k,i,j) = diff_val;
                    r_diff_elirl(m,l,k,i,j) = r_diff;
                end
                avg_val_elirl(m,l,k,j) = mean(val_elirl(m,l,k,:,j));
                err_val_elirl(m,l,k,j) = std(val_elirl(m,l,k,:,j))/sqrt(num_tasks);
                avg_val_diff_elirl(m,l,k,j) = mean(val_diff_elirl(m,l,k,:,j));
                err_val_diff_elirl(m,l,k,j) = std(val_diff_elirl(m,l,k,:,j))/sqrt(num_tasks);
                avg_r_diff_elirl(m,l,k,j) = mean(r_diff_elirl(m,l,k,:,j));
                err_r_diff_elirl(m,l,k,j) = std(r_diff_elirl(m,l,k,:,j))/sqrt(num_tasks);
            end
        end
    end  
end
